...
- `crates/bugwarden` — the binary: clap CLI, rmcp 3.4 MCP server, stdio and
...
- **Boundary.** Records go to the sinks the operator named
  ...
  of the unsupported-version line, which puts the whole forgery on the
  very next line of the same session (`debug!` since rmcp 3.3, so under a
  raised filter only) — so either can still carry text
...
- **The `guard.rule` encoding (issue #67).** `Access` carries
  ...
  a rule-less note later UPGRADED to `served_filtered` — a suppression, a
  GUARD redaction, a dropping scan — since that note outranks the grant
  and the worst-wins merge clears the rule with it (a `list_attachments`
  call that dropped private metadata is the plain case: the default
  granted the bug, and the record still names no rule). "Upgraded" is
  load-bearing: a rule-less note arriving at EQUAL rank may only fill an
  empty slot, so it clears nothing. `bug_info`'s `summary_view` is that
  case — the projected bug's `served_filtered` verdict and its rule are
  recorded together, then `summary_view` is named on top — and its record
  keeps `summaries-only`. A `redacted_fields` entry therefore does not by
  itself imply an absent rule; being the note that MOVED the verdict does.
  ONE carve-out on top of all that: the client-requested presentation
  windows (`comments_window`, `history_window`, `attachment_window`) ride
  `redacted_fields` beside the guard's own markers but are not guard
  decisions at all — the CLIENT asked for fewer lines — so
  `AuditCell::note_redacted_client_window` upgrades the verdict and never
  touches the rule, not even on the strict upgrade where the ordinary
  merge would clear it. Clearing it there reported a call a rule did
  grant as rule-less (an attachment served under `allow-desktop` with
  `head_lines: 3` recorded no rule), losing the attribution over a
  presentation choice. It is `merge`'s own body under a `keep_rule` flag,
  so the carve-out cannot drift from the merge it carves out of, and it is
  order-independent: a guard note landing after it merges by the ordinary
  rules. The guard-driven notes sitting right beside it — the I5 comment
  suppression in `bug_comments`, which DOES upgrade — still clear,
  deliberately: a suppression IS a decision, just not one a single rule
  made. Accepted cost, and the reason this is not a schema bump: the
  emitted bytes change inside `v: 1` for a scenario that already existed —
  a windowing `bug_history` / `bug_comments` / `download_attachment` call
  that wrote no `rule` now writes one. No field is added, removed or
  renamed and no discriminator exists (`v`, `policy_hash` and the
  `initialize` record are all unchanged), so a `v: 1` corpus spanning this
  build carries both encodings and a pre-upgrade rule-less
  `served_filtered` record may still have been granted by a named rule.
...
- Protocol revisions: `SUPPORTED_PROTOCOL_VERSIONS` (server.rs) lists what
  ...
  **An `initialize` request selects legacy semantics whatever version it
  ...
  SDK's `negotiate_protocol_version` (`service/server.rs`), which rmcp 3.3
  wraps as `ServerHandler::negotiate_initialize`. The override does not
  call it and agrees with it only while `DEFAULT_PROTOCOL_VERSION` is
  itself handshake-era, so re-read both on an rmcp bump. It records
...
- **rmcp trap — `Implementation::from_build_env()` names the SDK, not this
  ...
  *inside rmcp*, so a server built on it introduces itself as `rmcp` at the
  SDK's version. It is not opt-in: `ServerConfig::new()` (i.e.
...
- **rmcp trap — over stdio, `server/discover` CHOOSES the lifecycle before it is
  ...
  `notifications/cancelled` (no reply; its record is still written), can
  be answered after frames sent behind it, logs under the `serve_inner`
  span and gets rmcp's `response error` WARN on an error reply, and is
  abandoned when rmcp's 5 s drain runs out after stdin closes or an
  over-cap frame closes the transport — its upstream call can land with
  no reply and no record, the gap every later stdio call already had
  (#319).
  **Decision:** a probe is a probe and must not pick a lifecycle, so over
  ...
  of a `select!` (`service.rs:1418`) and drops that future whenever another arm
  ...
  (`model.rs:4684`) has no `DiscoverResult` arm; no audit record, no guard, no
...
- **rmcp trap — a panicking handler answers nothing, and rmcp catches
  ...
  `spawn_service_task` (`service.rs:1609`, a bare `tokio::spawn` at
  `:1328`) and DROPS the returned `JoinHandle`; the reply is sent from
  inside that task, at its end (`:1623`). Since rmcp 3.4 (#1263) that
  includes the first request of a stdio session opened without
  `initialize` (see the stdio trap above). There is no `catch_unwind`
  anywhere in rmcp 3.4. So a tool handler that panics unwinds its own task
...
- **Every `StreamableHttpServerConfig` field is accounted for below** — set by
  ...
  | field | rmcp 3.4 default | this build |
  |---|---|---|
  ...
  | `allowed_origins`, and the private flag `enforce_origin_validation()` sets | `[]`, flag unset, i.e. validation off | inherited, deliberately |
  ...
  **What one refused frame writes** (#272), measured on the shipped binary at
  ...
  because the empty default IS the disabled state: `validate_origin_header`
  skips the check for an empty list, so naming it would assert nothing the
  default does not already say. rmcp 3.4 (#1192) changed how: the skip now
  also requires a new private flag to be unset, which only
  `enforce_origin_validation()` sets and `disable_allowed_origins()` clears,
  and `origin_is_allowed` lost the empty-list shortcut it had in rmcp 3.2.
  Once validation is on, the same change answers a malformed or non-UTF-8
  Origin 403 where rmcp 3.2 answered 400; this build never turns it on.
  Measured under `--insecure-no-auth` on this build and its rmcp 3.2
  parent alike: an `initialize` POST with no Origin, a mismatched one,
  `null`, a malformed one or a non-UTF-8 one is answered `200`. The
...
- Stdio serving: `let (stdin, stdout) = server.stdio_transport();` then `server.serve((stdin, stdout))`, never a bare `stdio()` — the same rule, and for the same reason, as never a bare `StreamableHttpServerConfig::default()`: rmcp's default reads a frame into an unbounded `Vec`, and the pair is built while `server` can still be borrowed because the frame cap comes from its own guard policy (#234, "rmcp 3.4 usage notes"). Stdio uses the same waiter across `serve` (the handshake wait an unused stdio container sits in) and `waiting`; a signal at either stage flushes the OTLP queue (when export is on) then `process::exit(0)`s, because rmcp's stdio transport reads stdin via `spawn_blocking` and that read does not unblock while the client holds the pipe — returning from `main` drops the runtime onto that thread. An over-cap frame is NOT that case and needs none of it: the refusal happens in a `poll_read` that just took a delivered chunk, so the blocking reader is idle with no `read(2)` outstanding and `main` returns normally, exit `1`, with the OTLP flush on the ordinary path.
...
- Integration tests (crates/bugwarden/tests/tools_wiremock.rs, wiremock +
  ...
  touched; and bug_history windowing (issue #142) — with neither head nor
  tail the response is still the bare array (no envelope), the
  scrub-before-window ordering pin (an entry the I14 scrub dropped occupies
  no window slot, so `head=2` shows two SURVIVING entries and the hidden id
  appears nowhere in the text; run the other way the window would keep the
  dropped entry and serve one), head/tail/both windows reporting what they
  omitted and an overlapping pair omitting nothing, and window params
  against a policy-denied bug drawing the uniform denial byte-identical to
  the unwindowed call, with no truncation envelope and `expect(0)` history
  requests (I2) — nothing stops a later edit from hoisting the envelope
  around the whole handler, which would tell a denied bug from a
  nonexistent one; and `bugs_quicksearch` grouping (issue #143) — a grouped
  response ungroups back to the flat one bug for bug in order (grouping is
  a projection, not a filter) including a key the default `include_fields`
  omits, a grouped call over a hiding upstream is byte-identical to the
  same call over an upstream missing those rows and never names the hidden
  product in a header (a dropped row leaves no group, header or count —
  I3), group order follows the SERVED window and not sort order (products
  Z/A/Z group Z first; sorting the groups is the easy mutation and the
  unit fixture disagrees with sorted order for the same reason), a
  summary-redacted row still buckets and keeps `_redacted`, the id-list
  advisory `note` sits beside `groups` with the same text it has beside
  `bugs`, an empty `group_by` returns the flat response unchanged, an
  unknown key is refused with `expect(0)` upstream requests, and
  `group_by` riding a failing search still draws the bare "Search failed"
  (I2). Header key order is pinned as sorted-with-`bugs`-first in a unit
  test, since the tool description promises clients a shape.
...
- Audit tests (crates/bugwarden/tests/audit_wiremock.rs + #[cfg(test)] in
  ...
  download does not read as a filtered serve; all three client-window
  notes (`attachment_window`, `history_window`, `comments_window`) keep
  the granting rule in the record, with a cell unit test pinning that
  ...
  `redacted_fields`: hoisting a field into a header is not a
  redaction), with `group_by` recorded verbatim in `params` since it
  is allowlisted; and the `guard.rule` encoding (issue #67) — a call decided by the policy
  ...
  a clean `served` with neither — an unconditional note would mark every
  windowed call filtered (#87/#88) — and the omitting arm additionally
  asserts `guard.rule` still names the rule that granted the call. On the
  `bug_comments` side that assertion is made twice over, because
  `default` is also what a misattribution would produce: once under a
...
- HTTP bearer tests (#[cfg(test)] in crates/bugwarden/src/http_auth.rs and
  ...
  "rmcp 3.4 usage notes" rests on; a scope-refused call leaves exactly ONE
