...
(starting without one is an error). One request is one newline-delimited
...
| `--allowed-hosts <HOST>` | `MCP_ALLOWED_HOSTS` | — | Hostname or `host:port` authority accepted in an inbound `Host` header (http transport only). Repeatable, and each value may list several hosts separated by commas (whitespace around commas is trimmed). An entry that is not a hostname or `host:port` is a startup error. Without any host no `Host` validation happens, so a client may address the server by any name |
...
| — | `RUST_LOG` | `info` | Tracing filter for the diagnostic log, which always goes to **stderr** — stdout belongs to the stdio transport. An unparsable value falls back to `info`. Whatever the filter, every FIELD of every line — the MCP library's own included — is cut at 1024 characters and has every control character and every mandatory line break escaped (U+0000–U+001F, U+007F, U+0080–U+009F, U+2028 and U+2029), so no single client string can fill the terminal, open an escape sequence in it, or end one line and start another. A value bugwarden logs from a client's own string is also QUOTED — `query="kernel"`, with `"` and `\` escaped inside it, and cut short enough that the closing quote always fits — so a reader that honours quotes cannot be handed a `key=value` pair the client wrote; grep these fields with the quotes. Three limits worth knowing. The bound is per field per line and says nothing about how many lines a client can cause. A reader that does NOT honour quotes is no better off: `grep 'status=HACKED'` still matches the line, and splitting it on spaces still yields the client's text as tokens. And two families of field are unquoted — the MCP library's `%`-formatted ones (its request id and the protocol version it echoes; its client-info field is quoted) and bugwarden's own `error`, `path`, `location` and `thread`, of which `error` carries Bugzilla's message text and so a client string at one remove — all bounded and escaped, none quoted. Panics are reported through this filter too, never straight to the descriptor: the first panic of a process is ERROR and every later one WARN, and neither carries the panic payload, so `RUST_LOG=error` reports only the first — use `RUST_LOG=error,bugwarden::panic_hook=warn` to keep every panic on stderr under an otherwise quiet filter. A stdio frame over the request cap is a WARN for the other half of the same rule — a refusal the server chose is not an operator's problem. The ERROR lines beside it are the MCP library's echo of the same reason and, before a completed handshake only, the server's own line reporting that the session is over. Removing the library's echo takes `rmcp::transport::async_rw=off` (a level here is a ceiling on verbosity, so `=warn` still lets ERROR through), and it is worth keeping: that is also where a genuine stdin read failure is reported, and after the handshake it is the only report of one |
...
configuration, a key misconfiguration, an unparsable `--allowed-hosts` list,
a Bugzilla client or transport error), `2` on a command-line usage error.
...
| `max_attachment_bytes` | integer | `2097152` (2 MiB) | Largest attachment `download_attachment` may return, and the same ceiling on what `add_attachment` may upload — both measured on the decoded size. `0` removes this cap. The transport's request limit is sized from this value on **both transports** — base64 expansion plus 1 MiB of headroom for the rest of the call, clamped to [4 MiB, 64 MiB] — so a canonical `add_attachment` at this cap fits through it (Bugzilla's own 65535-character comment limit keeps the other arguments well inside the headroom). It bounds the POST body over http, where an over-cap request is refused with a `413` and the server keeps serving, and one newline-delimited frame over stdio, where an over-cap frame closes the transport and the process exits `1` (nothing can answer it: the request id was inside the frame). The clamps mean two values are not honored by either transport: with `0` the transport keeps its 4 MiB bound, and a cap above ~47 MiB decoded is limited by the 64 MiB ceiling — "no policy cap" and "an enormous policy cap" must not become an unbounded request. Downloaded content is embedded base64 in the tool result and lands in the model's context — raise deliberately |
...
| `bug_history` | Change history of a bug, optionally only entries newer than a timestamp; `head`/`tail` keep only the first/last N entries — with either set the response is an envelope with a truncation summary instead of the bare array | `history` |
| `bug_comments` | Comments on a bug; private comments only per the private-comment gate. Optional windowing for large threads: `head` (first N — comment 0 is the report), `tail` (last N), `max_comment_chars` (per-comment character cap); when any is set the response is an object with `comments` and a `truncation` block instead of a bare array | `comments` |
| `bugs_quicksearch` | Bugzilla [quicksearch](https://bugzilla.readthedocs.io/en/latest/using/finding.html#quicksearch) — the `status` filter (default `ALL`) is prefixed to the query, and under any non-empty status a number in the query is content-matched, so it also matches bugs that merely mention it; with an empty `status` the query goes to Bugzilla bare, where a query of nothing but numbers is an exact id lookup (use `bug_info` for an exact set of known ids; an all-ids query gets an advisory note saying so). Paginated by `limit` (default 50) and `offset` (default 0) over the bugs the client may see. Optional `group_by` (comma-separated: `product`, `component`, `status`, `resolution`, `severity`, `priority`) buckets the results — the response becomes an object with `groups` instead of `bugs`, and each grouped field is reported once per group rather than once per bug. Results are silently policy-filtered (denied dropped, summary-only redacted) | per result: `read` / `summary` |
...
| `download_attachment` | Content of one attachment, alongside a JSON summary of its metadata: raster images (PNG, JPEG, GIF, WebP, BMP) as image content, everything else as a base64 blob resource under `bugzilla://attachment/{id}`. Text-ish attachments (`text/*` plus a small `application/*` allowlist) can be windowed with optional `head_lines`/`tail_lines`/`max_chars` — first/last lines of the decoded text, the result capped in characters, returned as a plain text block with truncation counts in the summary; the params are ignored (with a summary note) on non-text payloads and on one whose base64 will not decode. Windowed text is capped at 200 000 characters even without `max_chars`, because JSON escaping can inflate a payload well past its byte size. Capped by `max_attachment_bytes` on the FULL payload regardless of windowing; private attachments need the private-content double opt-in and, on download, a *missing* privacy flag counts as private | `attachments` on the owning bug |
