...
| `bug_comments` | Comments on a bug; private comments only per the private-comment gate. Optional windowing for large threads: `head` (first N — comment 0 is the report), `tail` (last N), `max_comment_length` (per-comment character cap); when any is set the response is an object with `comments` and a `truncation` block instead of a bare array | `comments` |
