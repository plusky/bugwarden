...
impl Cli {
    ...
    /// loopback spelling. rmcp 3.4.0's `parse_allowed_authority` would
...
/// True when `entry` is a Host authority rmcp 3.4.0 would compare to a
...
mod tests {
    ...
    fn unparsable_allowed_hosts_are_a_startup_error_named_in_the_message() {
        // Each of these is what rmcp 3.4.0 would store (or skip) without
